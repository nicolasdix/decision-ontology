...
from rdflib.namespace import PROV, DCTERMS

# ONTOLOGY NAMESPACE
DO_NS = Namespace("http://www.semanticweb.org/decision-ontology#")

def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python soc_transformer.py <path/to/file.soc>", file=sys.stderr)
        return 1
    ...
    DATA_NS = Namespace(f"http://www.semanticweb.org/decision-ontology/data/{dataset_id}/")
    ...
    # Provenance
    source_iri = URIRef(DATA_NS + f"file/{soc_path.name}")
    output_iri = URIRef(DATA_NS + f"file/{out_path.name}")
    activity_iri = URIRef(DATA_NS + f"mapping_run_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}")

    # Add Provenance
    g.add((source_iri, RDF.type, PROV.Entity))
    g.add((source_iri, DCTERMS.format, Literal("soc")))
    g.add((source_iri, DCTERMS.source, Literal("https://preflib.github.io/PrefLib-Jekyll/dataset/")))
    g.add((context_iri, RDF.type, PROV.Entity))
    g.add((context_iri, PROV.wasDerivedFrom, source_iri))
    g.add((activity_iri, RDF.type, PROV.Activity))
    g.add((activity_iri, PROV.generated, output_iri))
    g.add((activity_iri, PROV.used, source_iri))
    g.add((activity_iri, PROV.endedAtTime, Literal(now_time, datatype=XSD.dateTime)))
    g.add((output_iri, RDF.type, PROV.Entity))
    g.add((output_iri, DCTERMS.format, Literal("turtle")))
    g.add((output_iri, PROV.wasDerivedFrom, source_iri))
    g.add((output_iri, PROV.wasGeneratedBy, activity_iri))
